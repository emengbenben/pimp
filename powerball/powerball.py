	...
	def input(self,choice):
		...
		if self.status == 0:
			...
			if choice.isdigit():
				...
				self.tickets = choice.split(",")
				self.tickets = [int(i) for i in self.tickets]
				response = "Your tickets are: {}".format(self.tickets)
				response = response + self.options()
				self.status = 0

			elif (choice == "No") or (choice == "N") or (choice == "no") or (choice == "n"):
				self.tickets = self.GenerateRandom(self.num)
				response = "Your tickets are: {}".format(self.tickets)
				response = response + self.options()
				self.status = 0
			else:
